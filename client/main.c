...
static int dbgStatesApplied = 0;

/* dictionary (embedded words_data.h, sorted) */
static char **dictWords = NULL;
static int dictCount = 0;

/* ------------------------------------------------------------------ js glue */
EM_JS(void, js_join, (const char *room, const char *name), {  const g = window.__boggleGlue;
...
static void loadDict(void) {
    /* The word list is compiled into the wasm data segment (words_data.h,
     * generated by scripts/build.ts). DICT_DATA is writable on purpose: the
     * parse below replaces newlines with terminators in place. */
    char *data = DICT_DATA;
    dictWords = (char **)MemAlloc(sizeof(char *) * (size_t)DICT_COUNT);
    char *p = data;
    int n = 0;
    for (;;) {
        char *nl = strchr(p, '\n');
...
/* --------------------------------------------------------------- messaging */
static unsigned long msgSeq = 0;

static void sendJson(const char *json) {
    /* Inject a monotonic sequence number into every message. iroh-gossip's
     * PlumTree dedupes messages by CONTENT hash, so byte-identical repeats
     * (periodic hellos, claim retries) would be silently dropped. */
    static char buf[33000];
    snprintf(buf, sizeof buf, "{\"n\":%lu,%s", ++msgSeq, json + 1);
    js_send(buf);
}
...
static void applyState(const char *json, int n) {
    ...
    if (ptok >= 0 && t[ptok].type == JSMN_ARRAY) {
        ...
        int nn = 0;
        /* jsmn: this build counts OBJECT children as keys, so an object
         * element spans 1 + 2*size tokens; the next element follows. */
        int pobj = ptok + 1;
        for (int i = 0; i < t[ptok].size && nn < MAX_PLAYERS; i++) {
            int size = t[pobj].size;
            if (t[pobj].type == JSMN_OBJECT) {
                Player *np = &tmp[nn];
...
EMSCRIPTEN_KEEPALIVE const char *boggle_dbg(void) {
    ...
    int off = snprintf(buf, sizeof buf, "phase=%d players=%d room=%s me=%s total=%d words=%d cur=%s toast=%s st=%d",
                       (int)phase, playersN, room, meId, total, myWords, curWord, toast, dbgStatesApplied);
    for (int i = 0; i < playersN; i++) {
...
/* Test/debug helper: dictionary probe ("1"/"0" + word count). */
EMSCRIPTEN_KEEPALIVE const char *boggle_debug_dict(const char *word) {
    static char buf[64];
    snprintf(buf, sizeof buf, "%d/%d", dictHas(word), dictCount);
    return buf;
}

/* Test/debug helper: dump the word at a given index. */
EMSCRIPTEN_KEEPALIVE const char *boggle_debug_dictword(int idx) {
    if (idx < 0 || idx >= dictCount) return "";
    return dictWords[idx];
}
